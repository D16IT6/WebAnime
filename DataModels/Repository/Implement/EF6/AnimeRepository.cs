    ...
    public class AnimeRepository : IAnimeRepository
    {
        ...
        public async Task<Paging<AnimeItemViewModel>> AdvanceSearch(AnimeSearchViewModel model)
        {
            ...
            var result = Context.Animes
                .Where(x =>
                    !x.IsDeleted &&
                    (x.Title.ToLower().Contains(model.SearchTitle.ToLower()) || String.IsNullOrEmpty(model.SearchTitle)) &&
                    (x.OriginalTitle.ToLower().Contains(model.SearchTitle.ToLower()) || String.IsNullOrEmpty(model.SearchTitle)) &&
                    ...
                    (model.RatingHigher <= (x.Ratings.Any() ? x.Ratings.Sum(r => r.RatePoint) * 1.0 / x.Ratings.Count() : 1) || model.RatingHigher == 0)
